pipeline {
    ...
    stages {
        ...
        stage('Push Docker Image') {
            steps {
                withCredentials([string(credentialsId: Docker_Cred_ID, variable: 'Docker_Cred')]) {
                    script {
                        sh """
                        echo \$Docker_Cred | docker login -u saint473 --password-stdin ${DOCKER_REGISTRY}
